...
class LinkedList:
    ...
    def init_linked_list(self, data: Iterable):
        ...
        for i in range(len(self.list_nodes)-1):
            left_node = self.list_nodes_nodes[i]
            right_node = self.list_nodes_nodes[i+1]
            self.linked_nodes(left_node, right_node)# TODO составьте алгоритм, который свяжет узлы

    # TODO каким должен быть этот метод? статическим
    @staticmethod
